...
import chargeLogoImg from "./real_charge_logo.png";
import "./main_section.css";
import CharactersCards from "./Characters-Cards";
import { showMarvelCharacters, searchMarvelCharacter } from "./main_section.services";
...
export default function AllCharacters () {
    ...
    useEffect(() => {
        if (inputName) {
            try {
                searchMarvelCharacter(inputName, offsetNumber)
                    .then(resp => {
                        setTotal(resp.data.data.total)                       
                        setMarvelCharacters(resp.data.data.results);          
                    })    
            } catch (error) {
                console.log(error);
            }
            ...
            try {
                showMarvelCharacters(offsetNumber)
                    .then(resp => {
                        setTotal(resp.data.data.total)
                        setMarvelCharacters(resp.data.data.results);          
                    })                     
    ...
    return ( 
            ...
            <div className="body_container"> 
                ...
                {marvelCharacters ?
                    marvelCharacters.map((element) => {
                        return (
                                <CharactersCards charactersData={element} key={element.id} />
                        );
                    })
                    :
                    <div className="chargeLogo_container">
                        <img src={chargeLogoImg} alt="charge Logo" />
                    </div>
                }
                </div>
                
                <div className="body_container_pagination">
                    <div className="body_container_pagination_2">
                        {marvelCharacters && 
                            <Pagination total={total} offsetNumber={offsetNumber} setOffsetNumber={setOffsetNumber} />
                        }
                    </div>
                </div>
